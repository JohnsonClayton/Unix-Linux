...
//int listDirs(int argc, string** argv, int a_set, int l_set, int r_set) {
int listDirs(vector<string> dir_vec, int a_set, int l_set, int r_set) {
	//printf("\n~~~ listDirs entered ~~~\n\targc: %d\n\targv[0]: %s\n", argc, argv[0]);
	DIR* head_dir;
	struct dirent* dp;
	int errno;
	...
	//	if(dir_vec.at(i) != NULL) {
			head_dir = opendir(dir_vec.at(i).c_str());
			if(head_dir) {
				printf("\n\tListing %s\n", dir_vec.at(i).c_str());
			}
			if(l_set && head_dir) {
...
//		head_dir = opendir(argv[i]);
	...
	//		if((dp = readdir(head_dir)) != NULL) {
				...
				if(r_set && S_ISDIR(stat_block.st_mode) && dp->d_name[0] != '.') {
					//if((sizeof(*new_args) / sizeof(string)) >= 100) {
					//	*new_args = malloc(sizeof(*new_args) + sizeof(string) * 20);
					//}	
					...
					++new_argsc;*/
					new_vec.push_back(dir_vec.at(i) + "/" + dp->d_name);

				}

				if(l_set) {
					if(S_ISDIR(stat_block.st_mode)) {
						printf("%d d", (int) dir_vec.size());
					} else {
						printf("-");
					}
					...
					if(dp->d_name[0] != 46) { // If the first stringacter is not a period, then show it 
						printf("%s \n", dp->d_name);
	...
	//		else {
				if(new_argsc) {
					return listDirs(new_vec, a_set, l_set, r_set);
				}
				if(errno == 0) {
					closedir(head_dir);
					return 1;
...
void setArgs(vector <string> dir_vec, int argc, char** argv,  int* a_set, int* l_set, int* r_set) {
